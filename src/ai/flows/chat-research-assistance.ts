...
const chatResearchAssistanceFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    **Core Instructions:**
    ...
    {{#if memory}}
    **User's Saved Memories & Facts:**
    You have access to the user's saved memories. You MUST consult this information to provide more personalized and context-aware responses. Do not mention the act of looking up the memory unless the user explicitly asks about it.
    {{#each memory}}
    - {{{this}}}
    {{/each}}
    {{/if}}
    ...
    {{#if contextualText}}
    **Contextual Information from File:**
    The user has provided a file. Use the following extracted text as the primary context for your response.
    ---
    {{{contextualText}}}
    ---
    {{/if}}
    ...
    **Conversation History:**
    ...
    const {output} = await ai.generate({
      ...
      context: {
        isFunChat: input.isFunChat,
        prompt: input.prompt,
        memory: input.memory,
        isDeepResearch: input.isDeepResearch,
