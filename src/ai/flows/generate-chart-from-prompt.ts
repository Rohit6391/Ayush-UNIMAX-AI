...
const prompt = ai.definePrompt({
  ...
  **Instructions:**
  ...
  2.  **Data Array**: Create a 'data' array of objects.
      *   For 'bar' and 'line' charts, each object should have a key for the x-axis (e.g., 'month', 'category') and one or more keys for the y-axis values.
      *   For 'pie' charts, each object must have a 'name' key for the label and a 'value' key for the numerical value.
  3.  **Config Object**: Create a 'config' object.
      *   This object maps data keys to their visual representation.
      *   For example, for a bar chart with data key 'sales', the config might look like: \`{"sales": {"label": "Sales", "color": "hsl(var(--chart-1))"}}\`
