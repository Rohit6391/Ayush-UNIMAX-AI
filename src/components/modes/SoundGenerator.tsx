...
export function SoundGenerator({ mode }: { mode: any }) {
    ...
    const handleGenerate = async () => {
        if (!prompt.trim()) { setError('Please enter a sound description.'); return; }
        setIsLoading(true); setError(''); setComposition(null); cleanup();
        
        try {
            const result = await generateSound({ prompt });
            setComposition(result.composition);
            addHistoryItem('sound_generator', prompt, `Generated a sound with a ${result.composition.length}-note melody.`);
        } catch (err: any) {
            setError(`Sound creation failed: ${err.message}.`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <ModeWrapper mode={mode}>
            <Textarea 
                ...
                placeholder="e.g., A short, happy notification sound..." 
            ...
            <Button onClick={handleGenerate} disabled={isLoading || isPlaying} className="w-full mt-4">
                {isLoading ? <><Settings className="animate-spin mr-2" /> Generating...</> : 'Generate Sound'}
            </Button>
                ...
                {composition && !isLoading && (
                    <Card className="text-left">
                        ...
                        <CardContent className="text-center">
                            <h3 className="font-semibold mb-4 text-lg">Sound Ready!</h3>
